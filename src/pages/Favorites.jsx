...
const Favorites = () => {
    ...
    const fetchFavorites = useCallback(async (pageNo = 1) => {
        if (!user) return;

        setLoading(true);
        try {
            const res = await api.get(`favorites/?auth_id=${user.id}&page=${pageNo}`);
    ...
    const toggleFavorite = async (productId) => {
        try {
            await api.post("favorites/toggle/", { product_id: productId, auth_id: user.id });
    ...
    return (
        <>
            ...
            <div className="container my-4 py-3">
                <h2 className="display-6 fw-bold text-center text-uppercase mb-4" style={{ color: "#198754" }}>
                    My Favorites
                </h2>
                {loading ? <LoadingGrid /> : (
                    <>
                                    ...
                                    >
                                        ...
                                        <div className="product-content">
                                            ...
                                            <div className="product-price-wrap">
                                                {hasOffer ? (
                                                    <>
                                                        <span className="product-price-offer">₹ {parseFloat(product.offer_price).toLocaleString()}</span>
